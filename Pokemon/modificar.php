<?php
session_start();

if( !isset($_SESSION["usuario"]) ){
    header("location:index.php");
    exit();
}
$usuario = $_SESSION["usuario"];
$id = $_GET["id"];

include_once("conexionBD.php");
$sql = "SELECT * FROM pokemon WHERE id = '$id'";
$result = mysqli_query($conexion, $sql);
$mostrar = mysqli_fetch_array($result);

?>

<!doctype html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Pokedex - Alta Pokemon</title>
    <script src="https://kit.fontawesome.com/534893fe33.js" crossorigin="anonymous"></script>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <link rel="stylesheet" href="css/estilo.css">
    <link rel="icon" type="image/x-icon" href="faviconpokemon.ico">
</head>
<body>
<header class="p-3 bg-danger text-white">
    <div class="container">
        <div class="row align-items-center">
            <div class="col">
                <a href="index.php"><img src="img/pokemon.png" style="width:150px;"/></a>
            </div>
            <div class="col">
                <h2>POKEDEX</h2>
            </div>
            <div class="col">
                <div class="row g-3">
                    <div class="col">
                        <h4>Bienvenido:  <?php echo"$usuario"?></h4>
                    </div>
                    <div class="col">
                        <button type="button"  class="btn btn-warning btn-sm boton_logeo"> <a style="text-decoration: none" href="logout.php">Cerrar sesión</a> </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>
<main>
    <a href="javascript:history.back()"> <img src="img/botonVolver.png" style="height: 50px" class="mt-2"></a>
    <h1 class="text-center">Modifique los datos que desee de <?php echo $mostrar['nombre'] ?></h1>
    <div class="container">
        <form action="procesarModificacion.php" class="col" method="POST" enctype="multipart/form-data">
            <div class="row">
                Numero ID: <input required type="text" name="numero_id">
            </div>
            <div class="row">
                Nombre: <input required type="text" name="nombre">
            </div>
            <div class="row">
                <label for="cars">Tipo:</label>

                <select name="tipo" id="tipo" required>
                    <option value="agua">Agua</option>
                    <option value="planta">Planta</option>
                    <option value="fuego">Fuego</option>
                    <option value="veneno">Veneno</option>
                </select>
            </div>
            <div class="row">
                Descripcion: <input required type="text" name="descripcion">
                <input type="hidden" name="id" value="<?php echo $id; ?>">
            </div>
            <div class="row">
                Añadir imagen: <input required name="archivo" id="archivo" type="file"/>
            </div>
            <br>
            <input type="submit" name="modificar" class="btn btn-secondary btn-lg" value="Enviar">

        </form>
    </div>
</main>
<footer class="mt-5">
    <div class="nav justify-content-center border-bottom pb-3 mb-3"></div>
    <p class="text-center">BuscaPoke Company- 2022</p>
</footer>
</body>
</html>

<?php

